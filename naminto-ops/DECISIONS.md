...
### D-21 — Chat en streaming : progression par étape (2026-08-22)

**Statut :** acceptée

**Contexte :** `POST /plan` répondait de façon synchrone unique (Reasoning Engine → Orchestrateur → sauvegarde) sans aucun retour pendant toute cette attente sauf « Naminto réfléchit… ». L'utilisateur a choisi de streamer, et a explicitement tranché **progression par étape** plutôt que texte brut du modèle token par token : le plan produit est du JSON structuré et chaque tâche exécute un script shell, pas du texte destiné à être lu en continu — streamer les tokens bruts n'aurait donné que du JSON illisible pendant qu'il arrive, sans bénéfice UX.

**Décision :** Nouveau type `TaskProgressEvent` (`task_start`/`task_complete`) dans `packages/naminto-core/src/agent.ts`, émis par `AgentOrchestrator.run()` via un 3ᵉ paramètre optionnel `onTaskEvent` (rétrocompatible — `SequentialAgentOrchestrator` encadre chaque appel `agent.run()`, y compris chaque tentative de `retryWithDebugAgent`, d'un `task_start`/`task_complete`). Pas de champ `index`/`total` : la boucle de débogage bornée (jusqu'à 3 tentatives) rend le nombre total d'étapes imprévisible à l'avance — le client accumule simplement chaque paire dans l'ordre d'arrivée, sûr puisque l'orchestrateur reste strictement séquentiel. `apps/api/src/plan/plan.controller.ts` : `POST /plan` devient un flux SSE via `@Sse()`, avec `@Post()` placé **au-dessus** de `@Sse()` — piège trouvé en implémentant : les décorateurs s'exécutent du plus proche de la méthode vers le plus loin, et `@Sse()` force inconditionnellement `METHOD_METADATA` à GET à chaque exécution ; il faut donc qu'il tourne en premier (juste au-dessus de la méthode) pour que `@Post()` écrase ensuite la méthode vers POST sans toucher au flag SSE séparé — dans l'ordre inverse, la route s'enregistre silencieusement en GET (404 au runtime sur un appel POST réel, aucune erreur à la compilation ni au démarrage). `@Sse()` choisi plutôt qu'un `@Res()` manuel : reste testable avec le même patron « appeler la méthode, obtenir une valeur » déjà utilisé partout dans ce code (même raison de testabilité que le choix de `@Redirect()` en D-18). Toute erreur en cours de route émet un événement `{type:'error'}` puis `subscriber.complete()` — jamais `subscriber.error()` — pour garder un format uniforme côté client, y compris pour les erreurs. Côté `apps/web/app/Chat.tsx`, lecture manuelle du flux via `response.body.getReader()` (pas d'`EventSource` natif — ne supporte ni POST ni en-tête `Authorization` personnalisé), découpage du buffer sur `\n\n`, extraction de la ligne `data: ...` (les frames Nest incluent aussi un `id:` auto-incrémenté, ignoré).

**Vérification en conditions réelles :** `npm run check` intégralement vert (94 tests côté `apps/api`/`apps/web` + tous les packages). `curl -N --no-buffer` sur `POST /plan` : trames SSE arrivant au fil du temps (`id: 1\ndata: {"type":"planning"}` puis `id: 2\ndata: {"type":"error",...}`), connexion fermée proprement (pas de blocage). Navigateur réel : soumission d'une intention → « Planification… » affiché → puis l'erreur de blocage crédit Anthropic déjà connue s'affiche cleanly, spinner arrêté, aucune erreur console. **Piège d'environnement rencontré pendant cette vérification, sans rapport avec le code livré** : un ancien process Next.js orphelin de l'ancien dépôt abandonné `D:\ADF\Naminto.AI` (qui possède son propre `.claude/launch.json` avec un nom de configuration `"web"` identique) squattait le port 3000, et l'outil de prévisualisation du navigateur a résolu ce launch.json-là au lieu de celui du dépôt courant — d'où un premier faux « bug » de parsing JSON observé contre la mauvaise application. Résolu en tuant le process orphelin et en démarrant le serveur `apps/web` directement dans le bon répertoire ; retest propre confirmant qu'il n'y avait aucun bug réel dans le code de streaming.

**Alternatives envisagées :** Streaming token par token du texte brut du modèle (explicitement écarté avec l'utilisateur — pas de bénéfice UX pour une sortie structurée). `EventSource` natif du navigateur (rejeté : ne supporte pas les requêtes POST ni un en-tête `Authorization` personnalisé, incompatible avec l'authentification par bearer token déjà en place). `@Res()` manuel avec `res.write()`/`res.end()` (rejeté : casse la testabilité directe des contrôleurs, même raison qu'en D-18).
