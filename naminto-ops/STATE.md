...
- Date : 2026-08-22
- Par : session Sortie de commande en direct (D-22) — extension du streaming D-21 : nouvelle variante `TaskProgressEvent` `{type:'task_output'; role; data}` forwardant les chunks `stdout`/`stderr` d'une tâche en cours vers le client, en direct. Le tuyau existait déjà à la couche la plus basse (`SandboxSession.execute()` acceptait déjà un `onOutput` appelé en direct, confirmé jusque dans `sandbox-e2b`) et `ShellScriptAgent` (base commune Coding/Testing/Debug) l'utilisait déjà, juste sans le remonter — `AgentRunContext` gagne `onOutput?: (data: string) => void`, branché par `SequentialAgentOrchestrator` sur `onTaskEvent`. Aucune modification nécessaire côté `plan.controller.ts` (le nouveau type d'événement traverse automatiquement l'union `PlanStreamEvent` existante). `Chat.tsx` affiche désormais un bloc de sortie qui grandit en direct sous la tâche en cours, pas seulement après sa fin. `npm run check` intégralement vert ; preuve principale par les nouveaux tests unitaires (le blocage crédit Anthropic empêche toujours une démonstration `curl`/navigateur bout en bout d'une vraie tâche, même limite que D-19/D-20/D-21). Détail : `DECISIONS.md` D-22.
- Session précédente : Chat en streaming — progression par étape (D-21) — voir `DECISIONS.md` D-21.

## Phase actuelle

**Le périmètre MVP minimal défini dans `DECISIONS.md` D-2 est intégralement complet**, et les chantiers au-delà du MVP le sont également : Project System (D-16, D-20), authentification avec isolation par compte réelle (D-14/D-16), refonte UI/UX (D-17), connexion Google (D-18, câblage vérifié, en attente d'un vrai client OAuth), historique et fichiers dans le chat (D-19), gestion de projets — renommer/supprimer (D-20), chat en streaming — progression par étape (D-21), sortie de commande en direct (D-22). Naminto Core + 4 Providers, Reasoning Engine, Agent Orchestrator, Coding/Testing/Debug Agent, sandbox réel (E2B), Memory System, File System, User System (mot de passe + Google), User Interface de chat présentable, protégée par connexion, scopée à un projet consultable/renommable/supprimable, avec historique et fichiers consultables, progression en direct pendant l'exécution — tout vérifié en conditions réelles dans la limite de ce qui est vérifiable sans comptes vendor réels. Deux points restent avant une démo bout-en-bout complète : du crédit Anthropic réel sur le compte, et un vrai client OAuth Google (tous deux déjà documentés, volontairement non résolus par l'utilisateur pour l'instant — voir « Blocages » ci-dessous).
...
19. ~~Sortie de commande en direct pendant une tâche~~ — fait (D-22), vérifié par tests unitaires (le blocage crédit Anthropic empêche une démonstration bout en bout d'une vraie tâche pour l'instant).
20. Retester tout le pipeline dès que du crédit Anthropic réel est disponible (y compris D-22 en conditions réelles), et tester la connexion Google de bout en bout dès qu'un vrai client OAuth est créé — deux vérifications en conditions réelles manquantes, tout le reste est intégralement complet.
