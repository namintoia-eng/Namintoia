...
- Date : 2026-08-22
- Par : session Connexion Google (D-18) — `UserSystem` gagne `authenticateExternal(ExternalIdentity)` (contrat vendor-neutre, `packages/naminto-core`) ; `LocalUserSystem` crée un compte sans mot de passe ou lie un compte existant par email vérifié. `apps/api/src/auth/google-oauth.service.ts` (nouveau) : mécanique Google (URL d'autorisation, échange de code, appel userinfo, rejet si email non vérifié) + `state` anti-CSRF signé sans état serveur (HMAC, `AUTH_STATE_SECRET`). Deux routes `GET /auth/google` et `GET /auth/google/callback` (`@Redirect()`). `apps/web` : bouton "Continuer avec Google" dans `AuthForm.tsx` (navigation réelle, pas de fetch) ; `page.tsx` lit le jeton dans le fragment d'URL (`#token=`, jamais loggé) au retour de Google, ou un message d'erreur en query (`?error=`). `npm run check` intégralement vert (35/35, 19/19 build) après correction d'un bug réel trouvé en testant : `GoogleOAuthService` était injecté par type implicite dans `AuthController` au lieu d'un `@Inject()` explicite (seul cas de ce type dans ce codebase) — cassait silencieusement sous le transform esbuild de vitest (`this.googleOAuth` undefined) bien que ça aurait fonctionné en build Nest réel ; corrigé en alignant sur la convention `@Inject()` déjà utilisée partout ailleurs. Vérifié en conditions réelles avec des identifiants factices : `/auth/google` sans config → erreur claire ; avec config factice → 302 réel vers `accounts.google.com` (le clic du bouton dans un vrai navigateur atteint effectivement l'écran d'erreur réel de Google "invalid_client", preuve que le câblage est correct) ; `state` altéré rejeté sans jamais appeler Google ; `state` valide + code factice → appel réseau réel à Google, rejeté par Google (401), erreur propagée correctement. Détail : `DECISIONS.md` D-18.
- Session précédente : Refonte UI/UX (D-17) — voir `DECISIONS.md` D-17.

## Phase actuelle

**Le périmètre MVP minimal défini dans `DECISIONS.md` D-2 est intégralement complet**, et les chantiers au-delà du MVP le sont également : Project System (D-16), authentification avec isolation par compte réelle (D-14/D-16), refonte UI/UX (D-17), connexion Google (D-18, câblage vérifié, en attente d'un vrai client OAuth). Naminto Core + 4 Providers, Reasoning Engine, Agent Orchestrator, Coding/Testing/Debug Agent, sandbox réel (E2B), Memory System, File System, User System (mot de passe + Google), User Interface de chat présentable, protégée par connexion et scopée à un projet — tout vérifié en conditions réelles dans la limite de ce qui est vérifiable sans comptes vendor réels. Deux points restent avant une démo bout-en-bout complète : du crédit Anthropic réel sur le compte, et un vrai client OAuth Google (tous deux déjà documentés, volontairement non résolus par l'utilisateur pour l'instant — voir « Blocages » ci-dessous).
...
16. Retester tout le pipeline dès que du crédit Anthropic réel est disponible, et tester la connexion Google de bout en bout dès qu'un vrai client OAuth est créé — deux vérifications en conditions réelles manquantes, tout le reste est intégralement complet.
...
- **Nouveau** : `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`/`AUTH_STATE_SECRET` non configurées — le bouton "Continuer avec Google" est câblé et vérifié avec des identifiants factices (redirection réelle vers Google atteinte, `state` CSRF validé) mais ne peut pas compléter un vrai flux avant que l'utilisateur crée un client OAuth dans Google Cloud Console (Credentials → OAuth Client ID → Web application → redirect URI exactement `http://localhost:3001/auth/google/callback`) et fournisse les vraies valeurs — même situation que `ANTHROPIC_API_KEY`/`E2B_API_KEY`, pas un blocage à résoudre proactivement.
