...
- Date : 2026-08-22
- Par : session Historique et fichiers dans le chat (D-19) — nouvel endpoint `GET /plan/:projectId/file?path=...` (contenu d'un fichier, seule pièce manquante côté serveur). `Chat.tsx` gagne deux sections : Historique (tours passés dans des `<details>` dépliables, plus récent en premier, réutilise un nouveau composant `PlanResult.tsx` extrait du bloc jusqu'ici dupliqué en dur) et Fichiers (liste cliquable, contenu chargé à la demande) — toutes deux rechargées après chaque soumission réussie, pas seulement au montage. `npm run check` intégralement vert (35/35, 19/19 build) après correction d'une erreur de typage (cast du mock `fetch` dans les tests). Vérifié en conditions réelles : le blocage crédit Anthropic empêchant tout `/plan` réel de réussir, des données ont été semées directement dans le stockage fichier (`.naminto/memory`, `.naminto/files`) au format exact écrit par `FileMemoryStore`/`LocalFileSystem` — le tour d'historique s'affiche, se déplie vers le même `PlanResult` que l'affichage principal, le fichier se charge et se referme au clic. Détail : `DECISIONS.md` D-19.
- Session précédente : Connexion Google (D-18) — voir `DECISIONS.md` D-18.

## Phase actuelle

**Le périmètre MVP minimal défini dans `DECISIONS.md` D-2 est intégralement complet**, et les chantiers au-delà du MVP le sont également : Project System (D-16), authentification avec isolation par compte réelle (D-14/D-16), refonte UI/UX (D-17), connexion Google (D-18, câblage vérifié, en attente d'un vrai client OAuth), historique et fichiers dans le chat (D-19). Naminto Core + 4 Providers, Reasoning Engine, Agent Orchestrator, Coding/Testing/Debug Agent, sandbox réel (E2B), Memory System, File System, User System (mot de passe + Google), User Interface de chat présentable, protégée par connexion, scopée à un projet, avec historique et fichiers consultables — tout vérifié en conditions réelles dans la limite de ce qui est vérifiable sans comptes vendor réels. Deux points restent avant une démo bout-en-bout complète : du crédit Anthropic réel sur le compte, et un vrai client OAuth Google (tous deux déjà documentés, volontairement non résolus par l'utilisateur pour l'instant — voir « Blocages » ci-dessous).
...
- [ ] Hors MVP (Phase 2+, voir D-2) : Design Agent, Architecture Agent, Research Agent, Deployment Agent en agents autonomes séparés ; Security System avancé ; Billing System ; Credit System ; Administration ; renommage/suppression de projets ; bascule thème clair/sombre ; édition/suppression de fichiers depuis l'UI
...
17. Retester tout le pipeline dès que du crédit Anthropic réel est disponible, et tester la connexion Google de bout en bout dès qu'un vrai client OAuth est créé — deux vérifications en conditions réelles manquantes, tout le reste est intégralement complet.
