...
- Date : 2026-08-22
- Par : session Gestion de projets (D-20) — `ProjectSystem` gagne `renameProject`/`deleteProject` ; `MemoryStore`/`FileSystem` gagnent chacun `deleteProject(projectId)` pour une **cascade de suppression** (sans ça, supprimer un projet aurait laissé son historique et ses fichiers orphelins indéfiniment sur disque). Routes `PATCH`/`DELETE /projects/:id` avec la même vérification de propriété (404) que le reste de l'API. `ProjectPicker.tsx` gagne une édition inline du nom et une confirmation de suppression intégrée à l'interface (pas de `window.confirm()` natif). `npm run check` intégralement vert (35/35, 19/19 build) après plusieurs allers-retours : deux échecs transitoires sans rapport (OOM/erreur disque sous forte charge parallèle, confirmés non reproductibles en isolation), un vrai oubli de mise à jour d'un mock `ProjectSystem` typé, et un vrai bug dans un test (mock `fetch` renvoyant une liste vide dès le premier appel). Vérifié en conditions réelles : renommage et suppression d'un projet existant dans un vrai navigateur ; inspection directe du stockage fichier confirmant que `.naminto/memory`/`.naminto/files` sont bien vidés du projet supprimé (données semées lors de la vérification D-19) ; `GET /plan/:projectId` sur l'ancien id renvoie 404 après suppression. Détail : `DECISIONS.md` D-20.
- Session précédente : Historique et fichiers dans le chat (D-19) — voir `DECISIONS.md` D-19.

## Phase actuelle

**Le périmètre MVP minimal défini dans `DECISIONS.md` D-2 est intégralement complet**, et les chantiers au-delà du MVP le sont également : Project System (D-16, D-20), authentification avec isolation par compte réelle (D-14/D-16), refonte UI/UX (D-17), connexion Google (D-18, câblage vérifié, en attente d'un vrai client OAuth), historique et fichiers dans le chat (D-19), gestion de projets — renommer/supprimer (D-20). Naminto Core + 4 Providers, Reasoning Engine, Agent Orchestrator, Coding/Testing/Debug Agent, sandbox réel (E2B), Memory System, File System, User System (mot de passe + Google), User Interface de chat présentable, protégée par connexion, scopée à un projet consultable/renommable/supprimable, avec historique et fichiers consultables — tout vérifié en conditions réelles dans la limite de ce qui est vérifiable sans comptes vendor réels. Deux points restent avant une démo bout-en-bout complète : du crédit Anthropic réel sur le compte, et un vrai client OAuth Google (tous deux déjà documentés, volontairement non résolus par l'utilisateur pour l'instant — voir « Blocages » ci-dessous).
...
18. Retester tout le pipeline dès que du crédit Anthropic réel est disponible, et tester la connexion Google de bout en bout dès qu'un vrai client OAuth est créé — deux vérifications en conditions réelles manquantes, tout le reste est intégralement complet.
